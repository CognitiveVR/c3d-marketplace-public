...
| --- | --- | --- |
| Unity | `references/unity_sdk_reference.md` | Unity 2020+ VR/AR/MR projects |
| WebXR | `references/webxr_sdk_reference.md` | Three.js, Mattercraft, Wonderland Engine, PlayCanvas, Babylon.js, A-Frame, plain WebXR/WebGL |
...
Never answer an implementation question with generic Cognitive3D knowledge when the SDK is unknown — the APIs differ in shape (WebXR custom events take a position argument, Unity's do not), the workflows differ in kind (Unity Editor tooling versus web app uploads), and several features exist on one target and not the other. Ask, or infer from the project, then route.
...
Never read, log, store, or output API keys, developer keys, SSO secrets, or credentials from any source — Unity's `Cognitive3D_Preferences`, a WebXR `settings.js` or `.env`, environment variables, config files, build scripts, or any other file that may contain secrets. If the SDK needs keys, instruct the developer to enter them themselves.
...
| `.uproject`, `Source/`, `.uasset` | **Unreal** — not covered by this skill, see below |
...
> Which Cognitive3D SDK is this project on — Unity, or WebXR? If WebXR, which framework: Three.js, Mattercraft, Wonderland, PlayCanvas, Babylon, or plain WebXR?

#### Record it and route on it

Put the SDK and, for WebXR, the framework at the top of the progress tracker (Step 3). Then:

- Load **only** the matching SDK reference when you reach Step 7. Never load both.
...
Cognitive3D also ships SDKs outside this skill's scope, Unreal Engine among them. If the project targets one of those, say so plainly, route to https://docs.cognitive3d.com/ for the correct SDK documentation, and offer the parts of this skill that still apply. The strategy layer — discovery, classification, primitives, phasing, naming, the business question map — is SDK-neutral and remains useful; only Step 7 technical routing does not.
...
- **Whether the team ships to more than one SDK** — if the same experience exists as both a Unity build and a WebXR build, plan them together. See the cross-SDK section of `references/sdk_capability_matrix.md`
...
1. **Against `references/sdk_capability_matrix.md`** — remove anything the target SDK or framework cannot do, and flag anything whose effort differs sharply from the other SDK (WebXR exit poll UI is the usual one). A plan containing an impossible row is worse than a smaller plan.
2. **Against `references/queryable_data.md`** (and `list_slicer_fields` when an MCP server is connected, to catch project-specific fields) — drop anything the platform already captures automatically: session duration, scene time, FPS, comfort, device context, geography. Make sure custom properties are typed so they are queryable (numbers as numbers). Spend the instrumentation budget on app-specific context only.
   ...
   **Caveat for WebXR projects:** `queryable_data.md` is generated from the Unity SDK's capture behaviour, so its automatic-sensor and device-field lists over-promise on WebXR (battery, CPU/GPU level, passthrough, wifi, biometrics, pupil diameter and multiplayer ping are all Unity-only). Where it and `sdk_capability_matrix.md` disagree, the capability matrix wins for WebXR. Dropping an instrumentation row because the platform "already captures it" is only safe once you have checked it against the right SDK.
...
When the developer asks **how** to implement something, load the SDK reference for the target identified in Step 0 — `references/unity_sdk_reference.md` **or** `references/webxr_sdk_reference.md`, never both — and route to the correct documentation page from there.

If the SDK is still unknown at this point, establish it before answering. A generic answer here is usually a wrong answer: the custom event API takes a position argument on WebXR and not on Unity, dynamic objects are Editor components on Unity and `userData` tags plus explicit registration on WebXR, and several features exist on only one target.

When routing implementation, identify whether the step is:

- **Authoring-tool workflow** — Unity Editor component setup and Inspector configuration, or Mattercraft behaviors and export hotkeys. Describe the steps; do not generate code, and do not offer to perform the step yourself.
- **Upload workflow** — scene geometry and dynamic object meshes. Unity uses in-engine tooling (Feature Builder); WebXR uses the Upload Web App at https://upload.cognitive3d.com with the Developer Key. Either way this is a human step you describe rather than perform, and in both SDKs the dynamic object mesh upload is **separate from the scene upload** and is routinely missed.
...
- **Check for an existing pattern for this feature.** On Unity and Mattercraft, dynamic objects are usually managed through editor components; the default recommendation should be to follow the project's existing pattern rather than introduce a parallel code-based one.
...
- Controller and boundary tracking active, allowing for the WebXR prerequisite that room-size data needs a `bounded-floor` session
- Offline/delayed upload behavior works (if needed and supported on this SDK)
- Dev and production traffic are separated

Add the SDK-specific validation items from the relevant reference file. The WebXR reference carries a troubleshooting quick table of the failure modes worth checking first; the Unity reference routes to https://docs.cognitive3d.com/unity/troubleshooting/ and its project validation page.
...
- **The app ships to an additional SDK** — a Unity experience gaining a WebXR build, or the reverse. Re-run Step 0 and the capability screen; the strategy usually carries over intact, but the plan rows may not
...
10. Scene export fidelity check (custom shaders on Unity; adapter export limits on WebXR)
...
3. Carrying a Unity-shaped plan onto a WebXR project without a capability screen
...
16. Letting event names or property keys diverge between a team's Unity and WebXR builds
...
9. **references/unity_sdk_reference.md** *or* **references/webxr_sdk_reference.md** — when the developer asks how to implement a recommendation. Load only the one matching the target identified in Step 0
