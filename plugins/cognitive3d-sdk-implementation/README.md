...
Supports **Unity**, **Unreal Engine**, **native Android XR** (Jetpack XR and Meta Spatial SDK), and **WebXR**. Capability varies sharply between targets, and the skill screens every plan against what the project's SDK can actually do.
...
- **SDK identification** — detects whether the project is Unity, Unreal, Android XR or WebXR, plus the WebXR framework, Android XR platform or Unreal authoring surface, then loads only the matching technical reference
...
- **Capability screening** — keeps plans inside what the target SDK can actually do: WebXR capability varies by adapter, several Unreal metrics come from opt-in components rather than automatic capture, and Android XR caps events at ten properties with no documented ExitPoll
