...
10. **Always set both `gte` and `lte` date bounds in slicer queries** — an open-ended range (e.g. `gte` only) has been observed to 502 on projects with a lot of data. This is about making the bounds explicit, not narrowing them: choose bounds that span your whole period of interest (a `gte` at or before the project's creation date is fine).
