...
description: "Expert guide for the Cognitive3D (C3D) REST API — constructing requests, suggesting endpoints, and parsing responses. Use for C3D session data, gaze/fixation tracking, sensors, dynamic objects, objectives, ExitPoll surveys, or building dashboards from the C3D backend. Triggers on c3d, cognitive3d, c3ddev, XR/VR analytics, API key setup, or slicer query construction."
tags:
  - analytics
  - data
  - query
...
To avoid exposing API keys in commands, **before making any API calls**, look for a value in the environment variable `C3D_API_KEY` and use that if it is present. Make sure not to print its value to the console. Check for its existence without printing it. In a curl command for example, use the variable rather than its value.

### Using a curl config file

There is an alternate option for configuring the API key. A `c3d_curlrc` file can be used. If the user wants to use this approach, use it instead of the environment variable with all curl commands:
```bash
curl --config ./c3d_curlrc https://api.cognitive3d.com/v0/...
```
(In this example, the c3d_curlrc would be in our active directory. Use the appropriate path if it is somewhere else.)

If the user wants to take this approach, they should create a file named `c3d_curlrc` in their working directory with the following content:
...
This also keeps the API key out of command history and console output. If you ask the user for the API key yourself and add it to curl commands the normal way as a header, it will be visible in console output from the user.
...
→ See `slicer_query_doc.md` for the full filter/aggregation syntax
...
**Full endpoint catalog** → `endpoints.md`
...
1. More advanced routes and howto guides for user data are coming soon.
2. More advanced routes and howto guides for objective results data are coming soon.
...
- **Slicer query response** (with `json0_key_y`): For 0D (no slice-bys), the value is at `aggregations.<name>.<op_name>.value`. For 1D, `aggregations.<name>.<op_name>.values` is an array of `{ "x": <bucket>, "value": <number> }`. For 2D, each entry has `{ "x": <bucket>, "values": { "<dim2_key>": <number>, ... } }` — dimension keys are named, not positional. See `output_types.md` for full details.
...
For Python, JavaScript, and C# request examples → `code-examples.md`

## Full Endpoint Catalog

For all endpoints organized by domain → `endpoints.md`

For a coverage table showing which endpoints have example responses → `endpoint-coverage.md`
When adding or updating any endpoint or example response in `endpoints.md`, update this table to match.
...
**Always use `nestedFieldName` for properties.** Some commonly indexed properties also support `unnestedFieldName`, which is faster — but only if the property is marked `unnested: true` in `slicer_fields.yaml`. If you use `unnestedFieldName` on a property that isn't unnested, the query will silently return wrong results. When in doubt, `nestedFieldName` always works.
...
Operations produce a single number by default. Add `sliceBys` at the aggregation level (not inside the operation) to bucket the data. See `slicer_query_doc.md` for full syntax and examples.
...
For **complex queries** (objective filters, user segmentation, event filters, compound filter logic, custom groups, histogram slice-bys) → `slicer_query_doc.md`

For **discovering what fields/properties/values exist** for a project before building a query → `slicer_api_guide.md`

For **looking up a specific field name**, property path, type, or display metadata → `slicer_fields.yaml`

For **choosing or parsing output formats** (legacy, json0_keyed, json0_list, json0_key_y) at different dimensionalities → `output_types.md`
